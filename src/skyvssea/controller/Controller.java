		...
		if (game.getCurrentGameState() != GameState.READY_TO_ATTACK) {
			...
			if (game.getCurrentGameState() == GameState.READY_TO_MOVE) {
				if (selectedTile.isHighlighted()) {
					...
					// Change piece location to a new tile. If selected tile is in the same
					// position, remain everything the same.
					if (!selectedTile.equals(previousSelectedTile)) {
						...
						PieceView pieceView = boardPane
								.getTileView(previousSelectedTile.getX(), previousSelectedTile.getY()).getPieceView();
